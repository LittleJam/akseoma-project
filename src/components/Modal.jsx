...
export default function Modal({
  ...
  return (
    <div
      ...
      className={`fixed inset-0 ${isViewer ? 'z-viewer bg-black/80' : 'z-modal bg-black/40'} flex items-start sm:items-center justify-center p-3 sm:p-6 overflow-y-auto animate-fade-in ${overlayClassName}`}
    >
      {/* Клик внутри окна не должен его закрывать */}
      <div
        onClick={e => e.stopPropagation()}
        className={`relative w-full ${SIZES[size]} my-auto flex flex-col animate-dialog-in ${panelClassName}`}
      >
        {children}
      </div>
    </div>
